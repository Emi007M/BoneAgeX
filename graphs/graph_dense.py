...
class GraphDense(Graph):
    ...
    def get_graph(self, graph=None):
        ...
        # ->> z gender_input i gender_size na , zakomentowane gender_input
        # dense_gender = tf.layers.dense(inputs=gender_input, units=gender_size,
        #                                activation=tf.nn.relu, name="dense_gender",
        ...
        dense1 = tf.layers.dense(inputs=bottleneck_input, units=dense_size, activation=activation_function, name="dense_1",
                                 bias_initializer=tf.constant_initializer(
        ...
        return Graph(bottleneck_input, ground_truth_input, None, None, final_tensor, MAE_scaled, train_step,
                     batch_size) # ew unscaleAgeT(final_tensor) i MAE nie scaled

    @staticmethod
    def get_graph_ops():
        return Graph("BottleneckInput:0", "GroundTruthInput:0",
                     None, None,
                     "output/BiasAdd:0", "MAE/MAE:0",
                     "train/train_step", "batch_size:0")
